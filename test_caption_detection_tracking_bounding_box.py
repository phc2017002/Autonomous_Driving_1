...
class AccidentVideoAnalyzer:
    ...
    def select_representative_frames(self, video_metadata: VideoMetadata, 
        ...
        """Select representative frames from the video"""
...
[{{"bbox_2d": [x1,y1,x2,y2], "object_label": "accident_object_1", "type": "description", "is_accident_related": true}}]"""
    ...
    def visualize_tracking_with_bboxes(self,
        ...
        if save_video and frames_with_tracks:
            ...
            print(f"Video saved to {video_path_bbox}")
    ...
    def process_video_complete(self,
        ...
        Output structure:
        accident_analysis_output/
        └── video_id/
            ├── initial_detections.json
            ├── complete_analysis.json
            ├── grounded_captions/
            │   ├── video_id_grounded_caption.json
            │   └── video_id_grounded_caption.txt
            ├── tracking_data/
            │   ├── bounding_boxes.json
            ...
            └── visualizations/
                ...
                ├── frame_*.png
                ├── tracking_video.mp4
                └── tracking_video_with_bboxes.mp4
        """
        ...
        # Step 1: Generate grounded caption
        if generate_caption:
            print(f"\n{'='*60}")
            ...
            try:
                ...
                selected_frames = self.select_representative_frames(
                    video_metadata, available_frames, max_frames=60
                )
                
                frame_numbers = [num for num, _ in selected_frames]
                print(f"Selected {len(selected_frames)} frames: {frame_numbers}")
        ...
        if track_objects and video_metadata.accident and self.enable_tracking:
            ...
            try:
                ...
                for offset in range(3):
                    ...
                    frame_path = Path(video_folder) / "images" / f"{frame_num:06d}.jpg"
                    
                    if not frame_path.exists():
                        frame_path = Path(video_folder) / f"{frame_num:06d}.jpg"
                ...
                if not any(initial_detections.values()):
                    ...
                    # Load frames for tracking
                    start_frame = video_metadata.accident_frame
...
def main():
    ...
    try:
        results = analyzer.process_video_complete(
            ...
            output_base_dir="accident_analysis_output",
        ...
        if enable_tracking:
            print("3. Accident object tracking with bounding boxes")
            print("4. Visualization frames and videos")
