...
class AccidentVideoAnalyzer:
    ...
    def __init__(self, 
                 ...
                 annotations_file: str = "cap_annotations.json"):
        """
        ...
        self.all_annotations = self.load_all_annotations()
    ...
    def load_all_annotations(self) -> Dict[str, Dict]:
        """Load all video annotations from cap_annotations.json"""
        annotations = {}
        ...
        if os.path.exists(self.annotations_file):
            ...
            try:
                ...
                # Convert list to dictionary keyed by video ID
                if isinstance(data, list):
                    for entry in data:
                        video_id = entry.get("Video", "")
                        if video_id:
                            annotations[video_id] = entry
            ...
            print(f"Warning: Annotations file {self.annotations_file} not found")
    ...
    def load_video_metadata(self, video_folder: Path) -> VideoMetadata:
        ...
        # First check if we have the annotation for this video in cap_annotations.json
        if video_id in self.all_annotations:
            ...
            if metadata.accident:
                ...
                print(f"  - Abnormal period: frames {metadata.abnormal_start}-{metadata.abnormal_end}")
                print(f"  - Cause: {metadata.causes[:100]}..." if len(metadata.causes) > 100 else f"  - Cause: {metadata.causes}")
        
            return metadata
    
    # Check embedded metadata (fallback for specific videos if needed)
    ...
    # Try to load from metadata.json in video folder (legacy support)
        metadata_file = video_folder / "metadata.json"
    
        if metadata_file.exists():
            print(f"Using metadata.json from video folder for {video_id}")
            ...
            print(f"Warning: No metadata found for {video_folder.name} in cap_annotations.json")
    ...
    def select_representative_frames(self, video_metadata: VideoMetadata, 
                                    ...
                                    max_frames: int = 30) -> List[Tuple[int, Path]]:
...
The incident occurs at night on an urban arterial road."""
    ...
    def detect_accident_objects_in_frame(self, 
        ...
        prompt = f"""Analyzing frame {frame_num}.
Detect accident or event-related objects:
- accident_object_1: primary object (e.g., vehicle or cause of impact)
...
def main():
    ...
    if not video_frames_path.exists():
        print(f"Error: {VIDEO_FRAMES_DIR} directory not found!")
        return
    
    # Get all subdirectories (video folders)
    video_folders = [d for d in video_frames_path.iterdir() if d.is_dir()]
    video_folders.sort()  # Sort for consistent processing order
    ...
    for idx, video_folder in enumerate(video_folders, 1):
        ...
        print("\n" + "="*70)
        print(f"PROCESSING VIDEO {idx}/{len(video_folders)}: {video_id}")
        print("="*70)
